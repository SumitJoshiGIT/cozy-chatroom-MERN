...
export default function (props) {
  ...
  const flag = messageItem.uid == userID.current;
   useEffect(()=>{
    if(messageItem.status=='⧖'){
      console.log('pending',(chatID.type=='user')?
      'createChatPrivate':
      'sendMessage',(chatID.type=='user')?
      'createChatPrivate':
      'sendMessage',messageItem);
      (socket.current.emit((chatID.type=='user')?
    'createChatPrivate':
    'sendMessage',
    {cid:messageItem.chat,content:messageItem.content,replace:messageItem._id,reply_to:messageItem.reply_to}))
    }
    },[])
  useEffect(() => {
  ...
  if(props.reply_data)console.log(props.reply_data,"REP")
  ...
  //<div style={{backgroundColor:((flag)?'#EEFFDE':'white'),maxWidth:'500px'}} className="relative top-1 h-3 w-2 message-clip"></div>
   return (
    <div id={messageItem._id} className={`w-full flex mt-3 justify-${flag ? "end" : "start"}`}>
        ...
        <div>
          ...
          {props.reply_to ? (
            ...
            <div  onClick={()=>{
             ...
             <span className="text-ellipses overflow-hidden max-w-sm h-16">Life is a dream come true in all aspects hue hue hueu hueh  expect fucking</span>
            </div>
          ) : null}
          ...
          <div className="float-right  sticky w-fit flex ml-1 h-fit flex-wrap items-end  text-gray-400 text-xs">
            ...
            <div className="text-xs ml-1">{messageItem.status}</div>
              ...
              <div onClick={()=>{contextref.current.style.display='none'}}
            ref={contextref}
